...
export function buildEmailVerificationEmail(params: {
  ...
  const content = `
    ...
    ${paragraph(greeting)}
    ${paragraph('Thanks for creating a Work Timer account. Please verify your email address by clicking the button below.')}

    ${button('Verify Email', params.verificationUrl)}

    ${paragraph('This link will expire in 24 hours. If you didn\'t create a Work Timer account, you can safely ignore this email.')}
    ${divider()}
  ...
  return {
    subject: 'Verify your Work Timer email',
    html: wrapInBaseLayout(content, 'Please verify your email to get started with Work Timer.'),
  }
}
