...
import { DollarSign, TrendingUp, Clock, Lock, Tag, FolderKanban } from 'lucide-react'
...
const PREVIEW_DATA = {
  ...
  items: [
    { id: '1', name: 'Development', color: '#6366f1', hours: 48.5, rate: 75, total: 3637.5 },
    { id: '2', name: 'Backend', color: '#10b981', hours: 35.2, rate: 100, total: 3520 },
    { id: '3', name: 'Design', color: '#f59e0b', hours: 22.8, rate: 60, total: 1368 },
...
export default async function EarningsPage({
  ...
  if (!premium) {
    ...
    const avgRate = p.items.length > 0
      ? p.items.reduce((s, item) => s + item.rate, 0) / p.items.length
      : 0
    return (
          ...
          <div className="blur-[2px] pointer-events-none select-none opacity-60 space-y-6">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {[
                ...
                { icon: <Tag className="h-4 w-4 text-purple-500" />, label: 'Tags', value: p.total_items },
          ...
          <div className="absolute inset-x-0 top-12 flex justify-center z-10 px-4">
            <div className="bg-white/95 dark:bg-[var(--dark-card)]/95 backdrop-blur-sm rounded-2xl shadow-2xl border border-stone-200 dark:border-[var(--dark-border)] p-8 text-center max-w-md w-full">
              ...
              <p className="text-sm text-stone-500 dark:text-stone-400 mb-6">
                Upgrade to Premium to track your earnings by tag, set hourly rates, and export detailed reports.
              </p>
              <a
                href="/billing"
  ...
  const { dateFrom: rawFrom, dateTo: rawTo, groupBy: rawGroupBy } = await searchParams
  ...
  const countLabel = groupBy === 'tag' ? 'Tags' : 'Projects'
  const CountIcon = groupBy === 'tag' ? Tag : FolderKanban
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <Card>
          <CardContent className="pt-5 pb-4">
            <div className="flex items-center gap-2 mb-1">
              <CountIcon className="h-4 w-4 text-purple-500" />
              <span className="text-xs text-stone-500 dark:text-stone-400">{countLabel}</span>
            </div>
            <p className="text-2xl font-bold text-stone-900 dark:text-stone-100">{data.total_items}</p>
          </CardContent>
        </Card>
      </div>
